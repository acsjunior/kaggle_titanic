...
subtitle: "Preparação dos dados para modelagem"
author:
...
```{r}
df_train <- read.csv("../Data/Kaggle/train.csv", na.strings = c("", NA), encoding = "UTF-8")

df_test <- read.csv("../Data/Kaggle/test.csv", na.strings = c("", NA), encoding = "UTF-8")
```
...
```{r}
df_all <- dummy_cols(df_all, select_columns = c("Pclass", "Embarked", "Title"), remove_selected_columns = T, remove_first_dummy = T)
```
